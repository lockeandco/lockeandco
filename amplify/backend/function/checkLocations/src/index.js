...
const jsonObject = xmlData => parser.parse(xmlData, xmlOptions)
		...
		R.compose(
			R.over(
				...
				R.ifElse(
					...
					R.compose(
						R.join('+'),
						R.map(R.compose(R.trim, R.replace('#', ''))),
						R.values,
			...
			R.over(
				...
				R.ifElse(
					...
					R.compose(
						R.mergeAll,
						R.map(x => Object.assign({}, {[x.subject_field_label]: x.value})),
...
async function updateDDb(item) {
	return await ddb
		.update(lcUpdateParameters(item))
		.promise()
		.then(x => x)
		.catch(error => error)
}

const getAddress = R.compose(R.path(['address']), R.head)
...
async function getAddressComponents(list, items = []) {
	if (isNilOrEmpty(list)) {
		...
		await fetch(url(R.path(['address'], R.head(list))))
			...
			.catch(R.compose(getAddressComponents([], items), R.tap(console.log)))
	}
}
